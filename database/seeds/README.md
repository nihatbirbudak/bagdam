# database/seeds — F2 katalog + ayar + admin seed'i

Statik prototipteki katalog verisini (`website/assets/products.js`) ve temel ayarları PostgreSQL'e yükler.
Kapsam: docs/BACKEND-PLANI.md §5 F2 / YOL-HARITASI F2 — *22 ürün · 15 üretici · 4 kategori (+legacyTab) · 2 tier ·
2 bölge · ProductLot (batch+why) · bu haftanın BoxTemplate'i · Setting commerce.\* · admin env'den*.
...
`SEED_OVERWRITE_SETTINGS=true` → Setting/SiteContent satırları seed değerlerine zorla çekilir (yalnız lokal/CI).
...
- **SiteContent**: yalnız 3 örnek anahtar gerçek metinle — `promoBar`, `footer`, `urunler.trust` (şema biçimi
  `@bagdam/shared` `SiteContentSchema`). `home.*`, `kutu.notes`, `manifesto.*`, `toptan.*`, `gunluk.*`, `sepet/uyelik.texts`,
  LegalDocument ve Post'lar **F5 içerik seed'inde**.
...
| Setting, SiteContent | Yalnız yoksa oluşturulur (`SEED_OVERWRITE_SETTINGS=true` ile ezilir). |
...
- **F5** içerik seed'i: SiteContent blokları + şemaları (kalan anahtarlar), LegalDocument v1 (8+3), 3 Post.
