...
- [ ] PayTR adaptörü (`PayTrProvider`: iFrame token init, callback hash + IP allowlist + WebhookEvent, Link API (PAYMENT_LINK), Kayıtlı Kart API (MIT, merchant onayı), iade) — UA modules/payment/gateways PayTR kodundan uyarla; kupon uygulama (Coupon/CouponRedemption, checkout quote) + admin Kuponlar ekranı; `POST /checkout/quote|checkout` (`$transaction`: doğrula → DeliveryDate rezerv → Order + lines snapshot [+ Subscription PENDING + cycle#1] → Payment → CF init); callback → PAID/ACTIVE/PaymentMethod; `GET /pay/:linkToken`
- [ ] sepet.html: PayTR iFrame konteyneri, özet quote'tan, buton metni, `?siparis=` (no-store); customerEmail readonly; Order geçişleri + iptal yan etkileri; sipariş onayı e-postası + LegalDocument kopyası; ekran 17 (Siparişler)
