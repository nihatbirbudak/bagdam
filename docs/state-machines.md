...
Çift çalışma koruması: tek instance + `SKIP LOCKED` + `Payment.conversationId` unique (aynı cycle/attempt ikinci kez tahsil edilemez). Sağlayıcı zaman aşımı: P PENDING kalır, `payments:reconcile` (F8'de eklendi — §14 #8) ya da sağlayıcı bildirimi ile kapanır.
...
- **Uygulama haritası (F8, 2026-08-21 — checkout + PayTR):** Checkout `modules/checkout/checkout.service.ts` tek `$transaction` (onaylar → kupon → DeliveryDate rezerv → Subscription PENDING + cycle#1 (`prepaidAmount` = kutu + ekstralar − indirim, **kargo hariç**; kargo `Order.shippingFee`) → Order + satır snapshot → Payment PENDING (`ord<orderNo>` + 4 rastgele) → sağlayıcı `initCheckout` → Consent satırları) · ödeme sonucu `PaymentsService.settlePayment` → `CheckoutCompletionService` (Order PAID, Subscription ACTIVE + cycle#1, PaymentMethod upsert, `CouponRedemption.usedCount++`, Notifier `order.paid`) — hem PayTR bildiriminden (`POST /payments/paytr/callback`, §8 callback SUCCESS) hem `payments:reconcile`'dan aynı yol · iade `PaymentsAdminService.refund` → Payment REFUNDED + (izinliyse) Order PAID|DELIVERED|DELIVERY_FAILED → REFUNDED (§1.2). Doğrulama: jest `__tests__/checkout/*`, `__tests__/coupons/*`, `__tests__/payments/paytr-*` + `tools/e2e-admin/run-f8.mjs` (rapor `report-f8.md`).
...
8. ~~**`payments:reconcile`:**~~ → **KARAR (F8, 2026-08-21): ayrı job eklendi.** `@Cron */15 * * * *` (`JobsService` → `CheckoutCompletionService.reconcile`): `createdAt <= now − 30 dk` olan açık (PENDING | REQUIRES_3DS) CHECKOUT/LINK ödemeleri sağlayıcıya sorulur (`PaymentProvider.retrieve`) → SUCCEEDED/FAILED sonucu `PaymentsService.settlePayment` ile normal yola girer (Order PAID / PAYMENT_FAILED). PayTR iFrame akışında sorgulanabilir bir sonuç yoksa bildirim beklenir; **24 saat** sonra Payment EXPIRED + Order CANCELLED + DeliveryDate rezerv iadesi. Durum makinesi değişmedi (PENDING→SUCCEEDED|FAILED|EXPIRED zaten var). Uygulama: `apps/api/src/modules/checkout/checkout-completion.service.ts`, `modules/jobs/jobs.service.ts`.
...
12. **Ops "teslim edilemedi" cycle tarafında:** Order DELIVERY_FAILED yeterli mi, CycleStatus'e DELIVERY_FAILED eklensin mi? Öneri: eklenmesin (ops Order üzerinden yönetir). — **F10 notu (2026-08-21):** teslimat bildirimleri (`order-shipped` / `order-delivered` / `order-delivery-failed`) tek huniden, `OrdersService.transition` üzerinden çıkıyor ve abonelik kutusu Order'larını da kapsıyor → müşteri bilgilendirmesi için cycle tarafında ayrı duruma **gerek kalmadı**; öneri güçlendi. İleride yine de istenirse enum'a **değer eklemek additive**'dir ([ADR-0021](adr/0021-sema-v1-donduruldu.md) serbest bırakır), ama `CycleStatus` geçiş tablosu ve ops listeleri de güncellenmeli.
