...
| Tasarım | Piksel parite: Playwright baseline staging'de; yalnız ADR-0003'teki izinli istisnalar (F8 sonrası fiilen 10 — liste `docs/SISTEM-DURUMU.md` F8 notlarında; ADR-0003 metnine işlenmeyi bekliyor). |
...
### F7 — Şema-b + fiyatlama + abonelik motoru (9 gün) — *UI'siz, testli* — ✅ LOKAL TAMAM (2026-08-20)
- [x] 1. gün F2b tasarım spike'ı + `0003_commerce` / `0004_raw_commerce` (Cart, Order, OrderLine, PaymentMethod, Payment, Refund, WebhookEvent, Subscription, SubscriptionCycle, CycleItem, SubscriptionEvent, SubscriptionCancellation) *(✓ `20260820183243_commerce` / `20260820183416_raw_commerce` (Prisma zaman damgalı; + minimal Coupon/CouponRedemption, F2a karşı ilişkiler, `orders_orderNo_seq` 1001, `payments_provider_pid_succeeded`); 37 model; shared enum/tip hizası; `docs/erd.md`)*
- [x] `PricingService` (tek kaynak; karışık sepet kind önceliği; kargo abone ‖ zone eşik; ilk-kutu/retention; DELTA) *(✓ `modules/pricing`: `quote(QuoteInput)` — shared computeQuote + Setting commerce.* (ADR-0018) + DeliveryZone + kullanıcı bağlamı (canlı abonelik / ilk-kutu hakkı / retention); `cycleCharge({cycleId} | açık biçim)` → CycleChargeQuote; couponCode F8'de; jest 9)*
- [x] `PaymentProvider` + `ManualProvider`; `ChargeStrategy` MIT / PAYMENT_LINK; `DeliveryDatesService` (generate TZ'li; atomik rezerv/iade; full) *(✓ `modules/payments`: arayüz + ManualProvider (`fail:` token / setOutcome) + PaymentProviderFactory (Setting → env → manual; PayTR F8 — ADR-0019) + MerchantInitiatedCharge/PaymentLinkCharge/ChargeStrategyResolver + PaymentsService (Payment/Refund/WebhookEvent idempotency) + public `GET /pay/:linkToken` JSON; `DeliveryDatesService` reserve/release/findOrCreateFor/nextFor/isLocked (409 DAY_FULL/DAY_CLOSED); jest 9 + 8)*
- [x] `SubscriptionsModule`: `cycles:ensure` (içerik = yayınlanmış BoxTemplate; şablon yoksa cycle üretilmez + ops uyarısı), `cycles:lock-and-charge`, `cycles:expire-payment-links`, dunning (+24 s/+72 s → UNPAID; 2 ardışık → PAST_DUE), skip/unskip (yılda 1, hak iade; cycle#1 atlanamaz), swap/pref/extras/merge-cart, freq/day/address/card PATCH, tek seferlik kutu → COMPLETED, cancel akışı (1:N, teklif, effectiveAt ≤7 g, confirm/abandon, refundDueAt), telafi ucu, SubscriptionEvent *(✓ `modules/subscriptions` (services subscriptions/cycles/cancellation/manual-checkout · repository FOR UPDATE SKIP LOCKED · mapper BootstrapSub) + `modules/orders` (Order çekirdeği, `/me/orders`, `/orders/:no/cancel`, `/admin/orders` + CSV) + `modules/jobs` (6 @Cron yalnız scheduler instance, CronLog, `POST /admin/jobs/:name/run {now?}`); dunning teslimat günü 08:00 sınırı (state-machines §14 #1 KARAR; LINK kolu dahil); **E entegrasyonu: `SUBSCRIPTIONS_DEPS` = `SubscriptionsDepsAdapter` → Pricing/Payments/Orders/Delivery gerçek servisleri, `ORDERS_DEPS` = DeliveryDatesService; admin `POST /admin/subscriptions` manuel checkout (F8 öncesi açılış yolu)**)*
- **Bitti sayılır:** Jest (UTC ve Europe/Istanbul): "11:59 ekstra kabul / 12:01 red", "2 haftalık takvim", "atla→geri al→kesim", "cycle#1 peşin + DELTA", "tek seferlik → COMPLETED", "iptal: kilitli cycle teslim", "UNPAID×2 → PAST_DUE", "PAYMENT_LINK süre dolunca UNPAID", "gün dolu → 409", 8 hafta fake-timer simülasyonu. → **lokal (ADR-0017):** API jest **28 suite/259 test** (gerçek DB; F7 suite'leri: engine 9 — DoD senaryolarının tümü · http 3 · capacity-template 2 · simulation 1 (56 gün × 5 job, `now` parametreli — fake-timer yerine) · manual-checkout 3 · pricing 9 · payments 9 · delivery-dates 8 · orders service/http; TZ=UTC, Europe/Istanbul, America/New_York aynı) ✓; shared vitest 117 ✓; **e2e F7 11/11** `tools/e2e-admin/run-f7.mjs` (API düzeyi: 3 müşteri → admin manuel checkout (MIT / PAYMENT_LINK / tek seferlik) → ensure → T0 kesimi → ops teslim → tek seferlik COMPLETED → atla/geri al → MIT tahsilat → link/expire/retry/08:00 SKIPPED(UNPAID) → iptal+retention → UNPAID×2 → PAST_DUE → kart düzelt → ACTIVE → iptal → CronLog; **temizlik ≡ başlangıç**) ✓. Rapor `tools/e2e-admin/report-f7.md`. UI/ops ekranları F9, gerçek sağlayıcı (PayTR) F8.

### F8 — Checkout + sipariş + PayTR + kupon (6 gün) — *ADR-0019: PayTR birincil; ADR-0010 iyzico → P2*
- [x] PayTR adaptörü (`PayTrProvider`: iFrame token init, callback hash + IP allowlist + WebhookEvent, Link API (PAYMENT_LINK), Kayıtlı Kart API (MIT, merchant onayı), iade) — UA modules/payment/gateways PayTR kodundan uyarla; kupon uygulama (Coupon/CouponRedemption, checkout quote) + admin Kuponlar ekranı; `POST /checkout/quote|checkout` (`$transaction`: doğrula → DeliveryDate rezerv → Order + lines snapshot [+ Subscription PENDING + cycle#1] → Payment → CF init); callback → PAID/ACTIVE/PaymentMethod; `GET /pay/:linkToken` *(✓ `modules/payments/providers/paytr/*`: saf hash yardımcıları (get-token · bildirim · Link · durum sorgu · iade · kayıtlı kart) + enjekte edilebilir `PAYTR_HTTP` (testlerde mock — **canlı/test API'sine istek atılmadı**) + `PayTrConfigService` (Setting `payment.*` → `.env PAYTR_*`) + `POST /payments/paytr/callback` (@Public @SkipCsrf @SkipThrottle; IP allowlist → hash → WebhookEvent idempotent → `settlePayment` → Order PAID/PAYMENT_FAILED + PaymentMethod + Notifier; düz metin "OK" / 400 "bad hash") + factory `paytr` kaydı; `storedCardEnabled` kapalıyken `chargeStoredCard` 503 `PROVIDER_FEATURE_DISABLED` → `ChargeStrategyResolver` PAYMENT_LINK'e düşer · `modules/coupons` (validate + admin CRUD + `CouponRedemption`; PAID'de `usedCount++`, iptal/iade geri alır) + `PricingService.quoteWithCoupon` · `modules/checkout` (`POST /checkout/quote` @Public · `POST /checkout` tek `$transaction`; `GET /orders/:no/status`; `payments:reconcile` @Cron 15 dk; `mail.order-paid`) · `GET /pay/:linkToken` F7'den)*
- [x] sepet.html: PayTR iFrame konteyneri, özet quote'tan, buton metni, `?siparis=` (no-store); customerEmail readonly; Order geçişleri + iptal yan etkileri; sipariş onayı e-postası + LegalDocument kopyası; ekran 17 (Siparişler) *(✓ `views/sepet.hbs` (`#paymentFrame` iFrame konteyneri — prototip kart formunun yerinde · özet `POST /checkout/quote`'tan, istemci hesap yapmaz · `#checkoutLegal` requiresAck onay kutuları · "siparişi tamamla — ödeme yükümlülüğü doğurur" · `#couponRow` kupon alanı · `GET /orders/:no/status` 2 s/120 s polling · `?siparis=&odeme=ok|hata` dönüş dalı) · `kutu.hbs` "aboneliği başlat" → sepet (markup değişmedi) · `uyelik.hbs` siparişler/kartlar/abonelik API'den, `bahceden_card`/`bahceden_orders` localStorage KALDIRILDI · admin ekran 17 (liste+filtre+CSV; detay: satırlar, ödemeler + `POST /admin/payments/:id/refund`, shared makineye göre durum düğmeleri, notlar, fatura) + Kuponlar ekranı + Ayarlar › Ödeme PayTR alanları/test modu uyarısı + Özet bugünkü sipariş/ciro kartı)*
- **Bitti sayılır:** staging sandbox'ta tekil ürün / tek seferlik kutu / abonelik ilk ödemesi (3DS + kart saklama) / sonraki cycle (MIT ve link) / webhook çift teslim IGNORED / iade; Consent kayıtları. → **lokal (ADR-0017): ✓** API jest **33 suite/301 test** (F8: checkout.http 10 · coupons 6 · paytr-hash · paytr-provider (mock HTTP) · paytr-callback.http); admin vitest **23 dosya/120 test**; shared vitest 117; **Playwright parite 30/30 0 px (mask `#forgotNote`) + duman 14/14**; **e2e F8 17/17** `tools/e2e-admin/run-f8.mjs` (site + panel + API + psql: kupon → tekil sipariş PAID → durum geçişleri → CSV → iade → REFUNDED; abonelik checkout → ACTIVE + cycle#1 peşin (kargo hariç) → iptal/retention → CANCELLED; PayTR bildirimi: geçersiz hash 400 · IP allowlist 403 · geçerli hash → PAID · ikinci teslim IGNORED; **temizlik ≡ başlangıç**) — rapor `tools/e2e-admin/report-f8.md`. **AÇIK:** gerçek PayTR sandbox provası (3DS + kart saklama + iade) mağaza bilgisi gelince **F10b/F11 staging**'de; `payment.storedCardEnabled` PayTR yazılı teyidine kadar kapalı (abonelik tahsilatı PAYMENT_LINK).
...
## 5. (Tarihçe) F1 başlangıcı — 2026-08-20'de uygulandı
